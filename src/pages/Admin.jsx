...
export default function Admin() {
  ...
  const [metricas, setMetricas] = useState({
    ...
    totalPedidos: 0, pedidosPendientes: 0, pedidosDisputa: 0,
    ventasTotales: 0, comisionesTotales: 0,
  })
  ...
  const cargarDatos = async () => {
    setCargando(true)
    try {
      const { data: todasTiendas } = await supabase.from('stores').select('id, verificado, activo')
      ...
      const { data: todosPedidos } = await supabase
        .from('orders').select('id, total, comision_monto, estado, creado_en, buyer_name')
      ...
      const completados       = todosPedidos?.filter(p => p.estado === 'completado' || p.estado === 'entregado') || []
      ...
      const { data: tiendasPendData } = await supabase
        .from('stores').select('id, nombre, slug, ciudad, creado_en')
        .eq('verificado', false).eq('activo', true)
      ...
      setMetricas({
        ...
        ventasTotales, comisionesTotales,
      })
      setPedidosPendientes(pendientesPago.slice(0, 8))
  ...
  const confirmarPago = async (pedidoId) => {
    const { error } = await supabase.from('orders').update({ estado: 'pagado' }).eq('id', pedidoId)
    if (!error) {
      setPedidosPendientes(prev => prev.filter(p => p.id !== pedidoId))
      setMetricas(prev => ({ ...prev, pedidosPendientes: prev.pedidosPendientes - 1 }))
    }
  }

  const verificarTienda = async (tiendaId) => {
    const { error } = await supabase.from('stores').update({ verificado: true }).eq('id', tiendaId)
    if (!error) {
      setTiendasPendientes(prev => prev.filter(t => t.id !== tiendaId))
      setMetricas(prev => ({ ...prev, tiendasPendientes: prev.tiendasPendientes - 1 }))
    }
  }
  ...
  if (cargando) {
    return (
      <div className="admin"><div className="container">
    ...
    <div className="admin">
      <div className="container">
        ...
        <div className="admin__encabezado">
          ...
          <button className="admin__btn-recargar" onClick={cargarDatos} type="button">
            Actualizar datos
          </button>
        </div>
        ...
        <div className="admin__metricas">
          <div className="admin__metrica">
            <span className="admin__metrica-valor">{formatearPrecio(metricas.comisionesTotales)}</span>
            <span className="admin__metrica-label">Comisiones ganadas</span>
          </div>
          <div className="admin__metrica">
            <span className="admin__metrica-valor">{formatearPrecio(metricas.ventasTotales)}</span>
          ...
          <div className="admin__alertas">
            {metricas.pedidosPendientes > 0 && (
              <div className="admin__alerta admin__alerta--pago">
                <strong>{metricas.pedidosPendientes}</strong> pedidos esperando confirmacion de pago
              </div>
            )}
            {metricas.pedidosDisputa > 0 && (
              <div className="admin__alerta admin__alerta--disputa">
                <strong>{metricas.pedidosDisputa}</strong> disputas abiertas requieren atencion
              </div>
            )}
          </div>
        )}
            ...
            <div className="admin__bloque">
              <div className="admin__bloque-header">
                <h2 className="admin__bloque-titulo">
                  Pagos por confirmar
                  {metricas.pedidosPendientes > 0 && (
                    <span className="admin__badge admin__badge--urgente">{metricas.pedidosPendientes}</span>
                  )}
                </h2>
                <Link to="/admin/pedidos" className="admin__ver-todos">Ver todos</Link>
              </div>
              {pedidosPendientes.length === 0 ? (
                <p className="admin__vacio">No hay pagos pendientes.</p>
              ) : (
                <div className="admin__lista">
                  {pedidosPendientes.map(pedido => (
                    <div key={pedido.id} className="admin__item">
                      <div className="admin__item-info">
                        <code className="admin__item-ref">#{pedido.id.slice(0, 6).toUpperCase()}</code>
                        <span className="admin__item-nombre">{pedido.buyer_name}</span>
                        <span className="admin__item-fecha">{formatearFecha(pedido.creado_en)}</span>
                      </div>
                      <div className="admin__item-acciones">
                        <span className="admin__item-total">{formatearPrecio(pedido.total)}</span>
                        <button className="admin__btn-confirmar" onClick={() => confirmarPago(pedido.id)} type="button">
                          Confirmar pago
                        </button>
                      </div>
                    </div>
              ...
              <div className="admin__bloque-header">
                <h2 className="admin__bloque-titulo">
                  ...
                  {metricas.tiendasPendientes > 0 && (
                    <span className="admin__badge admin__badge--normal">{metricas.tiendasPendientes}</span>
                  )}
                </h2>
                <Link to="/admin/tiendas" className="admin__ver-todos">Ver todas</Link>
              </div>
              {tiendasPendientes.length === 0 ? (
                <p className="admin__vacio">No hay tiendas pendientes.</p>
              ) : (
                <div className="admin__lista">
                  {tiendasPendientes.map(tienda => (
                    <div key={tienda.id} className="admin__item">
                      <div className="admin__item-info">
                        <div className="admin__item-avatar">{tienda.nombre.charAt(0).toUpperCase()}</div>
                        <div>
                          <span className="admin__item-nombre">{tienda.nombre}</span>
                          {tienda.ciudad && <span className="admin__item-ciudad">{tienda.ciudad}</span>}
                        </div>
                      </div>
                      <div className="admin__item-acciones">
                        <button className="admin__btn-verificar" onClick={() => verificarTienda(tienda.id)} type="button">
                          Verificar
                        </button>
                        <a href={`/tienda/${tienda.slug}`} className="admin__btn-ver" target="_blank" rel="noopener noreferrer">
                          Ver tienda
                        </a>
                      </div>
                    </div>
